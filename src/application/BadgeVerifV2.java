...
public class BadgeVerifV2 {
	...
	public static void main(String args[]) throws InterruptedException, IOException {
		...
		final GpioPinDigitalOutput greenLED = gpio.provisionDigitalOutputPin(RaspiPin.GPIO_03, "greenLED",
				PinState.LOW);
						...
						if (value[0].equals(idBadge) && value[1].equals(IDZONE.toString())) {
							...
							do {
								...
								if (digiCodeWindow.isCorrect()) {
									...
									greenLED.toggle();
									snap.waitFor();
									greenLED.toggle();// led off after taking
														// video
		...
		try {
			...
			config.device("/dev/ttyUSB0").baud(Baud._2400).dataBits(DataBits._8).parity(Parity.NONE)
